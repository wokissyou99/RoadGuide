...
void population::GA(population &r)
{
	for (int k = 0; k < r.Maxevolution; k++)
	{
		...
		/*for (unsigned int i = 0; i < group.size(); i++)
		{
			for (unsigned int j = 0; j < r.group[0].Chromosome1.size(); j++)
			{
				cout << r.group[i].Chromosome1[j] << " ";
			}
			cout << endl;
		}*/
		...
		/*cout << "选择：" << endl;
		for (unsigned int i = 0; i < newGroup.size(); i++)
		{
			for (unsigned int j = 0; j < newGroup[0].Chromosome1.size(); j++)
			{
				cout << newGroup[i].Chromosome1[j] << " ";
			}
			cout << endl;
		}*/
		...
		r.group = newGroup;
		variation(r, f_avg);
		//cout << "第" << k << "次" << "迭代，最大适应度：" << r.group[0].Fitness << endl;
		/*for (unsigned int i = 0; i < r.group.size(); i++)
		{
				cout << r.group[i].Fitness << " ";
		}
		cout << endl;*/
	}
	cout << "最大适应度：" << r.group[0].Fitness << endl;
}

void graph::Init(string _str_path) {
	FILE* fp;
	fopen_s(&fp, "测试用例1.txt", "r");
	if (fp == NULL) {
